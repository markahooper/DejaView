...
# --------------- Inputs (with sample picker next to uploader) ---------------
...
if mode.startswith("Composite"):
    ...
    cols_upload = st.columns([1,1,2])
    with cols_upload[0]:
        use_sample_comp = st.checkbox("Use sample image", value=False, key="use_sample_comp")
    with cols_upload[1]:
        if use_sample_comp:
    ...
    cols_upload = st.columns([1,1,2])
    with cols_upload[0]:
        use_sample_pan = st.checkbox("Use sample image", value=False, key="use_sample_pan")
    with cols_upload[1]:
        if use_sample_pan:
            options = list_sample_images()
            if options:
        ...
        if mode == "Composite (auto-split)":
            ...
            if sample_choice_panels:
                with open(sample_choice_panels, "rb") as fh:
                    data = fh.read()
                # Use auto-split defaults to derive panels from the sample composite
                rgb = imread_rgb_from_bytes(data)
                panels = auto_split_panels(rgb, skip_top=0.05, min_area_frac=0.015, max_area_frac=0.7, target_max=max_size)
                input_preview = data  # show composite in the expander
                if len(panels) < 2:
