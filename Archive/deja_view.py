...
def build_text_suppression_mask(rgb):
    """
    Build a mask (255 = allowed for keypoints, 0 = suppressed) to downweight text-like regions.
    Uses MSER to detect likely text blobs, then dilates them.
    """
    ...
    allowed = cv2.bitwise_not(text_mask)
    return allowed

def orb_match_details(A, B, ratio=0.78, nfeatures=2000, fastThreshold=10, suppress_text=True):
    """
    Return (inliers, H, good_matches, kpa, kpb, mask) and draw matches with only inliers.
    When suppress_text=True, ORB keypoint detection ignores text-like regions (MSER-based).
    """
...
def draw_inlier_matches(A, B, kpa, kpb, good, mask, max_draw=120):
    """Return an RGB image with only inlier matches drawn between A and B (side-by-side)."""
...
def make_overlay_hull(A, B, H, inlier_ptsA, edge_color=(0,255,0), fill_color=(0,255,0), alpha=0.25, thickness=3):
    """Warp B into A's frame, color-match, blend, and draw a convex hull polygon around inlier region on A."""
    Ha, Wa = A.shape[:2]
...
:root {
  --ink: #57554e;   /* primary text */
  --accent: #b5b1ed;/* lavender */
  --sun: #fdc474;   /* amber */
}
html, body, [class*="css"]  {
